              ...
              {filtered.map(lead => (
                <div key={lead.id}
                  ...
                  style={{
                    ...
                    border: `1px solid ${selected?.id === lead.id ? 'rgba(0,212,255,0.3)' : 'var(--border)'}`,
                    cursor: 'pointer', transition: 'all 0.15s',
                    display: 'flex', alignItems: 'center', gap: 14
                  }}
                  onMouseEnter={e => { if (selected?.id !== lead.id) e.currentTarget.style.borderColor = 'rgba(0,212,255,0.2)'; }}
                  onMouseLeave={e => { if (selected?.id !== lead.id) e.currentTarget.style.borderColor = 'var(--border)'; }}
                >
                  <div style={{
                    width: 38, height: 38, borderRadius: 9, flexShrink: 0,
                    background: `${STATUS_COLORS[lead.status] || '#00d4ff'}15`,
                    border: `1px solid ${STATUS_COLORS[lead.status] || '#00d4ff'}25`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 14,
                    color: STATUS_COLORS[lead.status] || '#00d4ff'
                  }}>
                    {lead.name.charAt(0)}
                  </div>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {lead.email_body && (
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 14px' }}>
            ...
            <div style={{ fontSize: 10, color: '#00d4ff', marginBottom: 4 }}>✉ Email Subject</div>
          ...
          <div>
            ...
            {msgs.map(m => (
              <div key={m.id} style={{
                ...
                background: m.direction === 'inbound' ? 'rgba(16,185,129,0.08)' : 'rgba(0,212,255,0.06)',
                border: `1px solid ${m.direction === 'inbound' ? 'rgba(16,185,129,0.2)' : 'rgba(0,212,255,0.15)'}`,
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <span style={{ fontSize: 10, color: m.direction === 'inbound' ? '#10b981' : '#00d4ff', fontFamily: 'var(--font-mono)' }}>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
          {progress && (
            <div style={{
              ...
              background: progress.error ? 'rgba(239,68,68,0.08)' : 'rgba(0,212,255,0.06)',
              border: `1px solid ${progress.error ? 'rgba(239,68,68,0.2)' : 'rgba(0,212,255,0.15)'}`,
            }}>
